"""Write tile-centric Parquet files for 2D ML training from StreamingTileGenerator2D.
...
Schema (9 columns):
    zoom (uint8), tile_x (uint16), tile_y (uint16), feature_id (uint32),
    geom_type (uint8), positions (large_binary), indices (large_binary),
    ring_lengths (list<uint32>), tags (map<utf8,utf8>)
"""
...
def _generate_parquet_inmemory(
    ...
    *,
    ...
    data = generator._collect_parquet_data(world_bounds, simplify)
...
def _dict_to_record_batch(data: dict) -> pa.RecordBatch:
    ...
    rl_arr = pa.array(
        [list(rl) for rl in data["ring_lengths"]],
    ...
    return pa.RecordBatch.from_arrays(
        [zoom_arr, tx_arr, ty_arr, fid_arr, gt_arr, pos_arr, idx_arr, rl_arr, tags_arr],
        schema=_parquet_schema(),
    )
...
def _parquet_schema() -> pa.Schema:
    return pa.schema(
        [
            ...
            pa.field("ring_lengths", pa.list_(pa.uint32())),
...
_MAX_ROW_GROUP_BYTES = 1_500_000_000

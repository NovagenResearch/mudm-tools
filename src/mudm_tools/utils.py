...
from mudm.model import Feature
import pydantic
...
class OmeMicrojsonModel:
    """Generate JSON of segmentations polygon using mudm python package.
    ...
    def polygons_to_microjson(  # noqa: PLR0915
        ...
        """Create mudm overlays in JSON Format."""
        import vaex
...
class CustomValidation(pydantic.BaseModel):
    ...
    def validate_file_path(cls, value):
        ...
        if not MuDM.model_validate(json.load(Path.open(Path(value)))):
            raise ValidationError(f"Not a valid MuDM {value.name}")
        if isinstance(value, str):
            return Path(value)
        return value
...
class MicrojsonBinaryModel(CustomValidation):
    """Generate binary masks from polygons stored in mudm format.
    ...
    def microjson_to_binary(self) -> None:
        ...
        logger.info(f"Converting mudm to binary mask: {self.file_path}")

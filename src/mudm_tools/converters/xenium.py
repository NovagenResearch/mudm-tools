...
def xenium_to_mudm(
    ...
    if "cell_id" not in cols:
        raise ValueError(
            f"cell_boundaries.parquet missing 'cell_id' column (got: {cols})"
        )
    if "vertex_x" not in cols or "vertex_y" not in cols:
        raise ValueError(
            f"cell_boundaries.parquet missing 'vertex_x'/'vertex_y' (got: {cols})"
        )
...
def _load_xenium_expression(
    ...
    raise FileNotFoundError(
        f"cell_feature_matrix path not understood: {matrix_path}"
    )

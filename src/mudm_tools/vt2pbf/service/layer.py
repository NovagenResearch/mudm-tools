...
class Layer:
    def __init__(self, tile_pbf, name: str, extend: int = EXTEND):
        ...
        self.value_indices: dict[Union[bool, str, int, float], int] = {}
    ...
    def add_feature(self, feature_info: dict, feature_id: int | None = None):
        self._validate_feature(feature_info)
        feature = Feature(
            layer=self,
            feature_type=feature_info["type"],
            feature_id=cast("int | None", feature_id or feature_info.get("id")),
        )
    ...
    def _validate_feature(feature_info: dict):
        if not Feature.REQUIRED_FIELDS.issubset(feature_info):
            raise InvalidFeatureError(
                f"Feature must provide all required fields: {Feature.REQUIRED_FIELDS}"
            )
